...
const enhance2 = enhancer => func => a => b => func(enhancer(a))(enhancer(b));
...
export const contains = normalized(query => value => (value).indexOf(query) !== -1);
export const exact = normalized(query => value => query === value);
export const startsWith = normalized(query => value => value.startsWith(query));
...
const score = query => link => {
  const keywordsScored = link.keywords.map(scoreKeyword(query)).sort(byScoreDesc);
  const bestKeyword = keywordsScored[0];
  const defaultKeyword = link.keywords[0];

  if (bestKeyword.score > 0) { // If any keywords matched, base the score on the best keyword.
    return {
      score: bestKeyword.score * USAGE_FREQUENCY_SCALE_FACTOR[link.usageFrequency],
      representativeKeyword: bestKeyword.value,
      value: link
    }
  } else { // If no keywords matched, base the score on the title, description, and target.
    return {
      score: scoreOthers(query)(link) * USAGE_FREQUENCY_SCALE_FACTOR[link.usageFrequency],
...
export const search = links => query => links
  .map(score(query))
